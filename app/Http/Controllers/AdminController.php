...
class AdminController extends Controller
{
    ...
    public function getAllRequest(){
        ...
        $data = VpnUser::select(['no_ticket','vpn_username','email','name'])
        ->leftJoin('users','vpn_users.user_id', '=','users.id')
        ->get();

        foreach ($dataAll as $key => $value) {
            $data[$key]['department_name'] = Department::select('name')->where('id',$dataAll[$key]['department_id'])->first()['name'];
            $data[$key]['acl_group_name'] = VpnUserGroup::select('acl_group_name')->where('department_id',$dataAll[$key]['department_id'])->first()['acl_group_name'];
    ...
    public function getAllVPNData(){
        $dataAll = VpnUser::select('*')
        ->leftJoin('users','vpn_users.user_id', '=','users.id')
        ->get();

        $data = VpnUser::select(['vpn_username','name','email'])
        ->leftJoin('users','vpn_users.user_id', '=','users.id')
        ->get();
        // dd($dataAll);
        ...
        foreach ($dataAll as $key => $value) {
            $data[$key]['department_name'] = Department::select('name')->where('id',$dataAll[$key]['department_id'])->first()['name'];
            $data[$key]['acl_group_name'] = VpnUserGroup::select('acl_group_name')->where('department_id',$dataAll[$key]['department_id'])->first()['acl_group_name'];
            $data[$key]['remote_address'] = $dataAll[$key]['remote_address'];
            $data[$key]['acl_group_name_allow'] = VpnUserGroup::select('acl_group_name_allow')->where('department_id',$dataAll[$key]['department_id'])->first()['acl_group_name_allow'];
    ...
    public function getDetailRequest($email){
    ...
    public function assignAddressVPN(Request $request){
        ...
        $vpn_user = VpnUser::where('id',$form['vpn_users_id'])->first();
        ...
        $data = $this->getDetailRequest($form['email']);
    ...
    public function createVPNMikroTik(Request $request){
        ...
        $userPPP = $client->sendSync(new RouterOS\Request('/ppp secret add name="'.$form['vpn_username'].'" password="binus123" service=l2tp profile=default local-address="'.$form['local_address'].'" remote-address="'.$form['remote_address'].'"'));

        $addAddrList = $client->sendSync(new RouterOS\Request('/ip firewall address-list add address='.$form['remote_address'].' list='.$form['acl_group_name'].''));
        
        foreach ($data['access_list_ticket'] as $key => $value) {
            $addAddrListAllow = $client->sendSync(new RouterOS\Request('/ip firewall address-list add address='.$value->address.' list='.$form['acl_group_name_allow'].''));
        ...
        foreach ($filterRule as $key => $value) {
            // if(strpos($form['acl_group_name'], $value('src-address-list'))){
            //     $existRule = true;
            //     break;
            // }
            $string .= ' '. $value('src-address-list');
        }
        // dd($string);
        // dd(strpos($string, $form['acl_group_name']));
        if(strpos($string, $form['acl_group_name']) == false){
            $filterRule = $client->sendSync(new RouterOS\Request('/ip firewall filter add action=accept chain=forward src-address-list='.$form['acl_group_name'].' dst-address-list='.$form['acl_group_name_allow'].' place-before='.$lastRule.' comment="'.$form['department_name'].'"'));
            $filterRuleAllow = $client->sendSync(new RouterOS\Request('/ip firewall filter add action=accept chain=forward src-address-list='.$form['acl_group_name_allow'].' dst-address-list='.$form['acl_group_name'].' place-before='.$lastRule.''));
        }
    }
}
